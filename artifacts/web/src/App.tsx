...
const [ownerFeedback, setOwnerFeedback] = useState<{ type: "success" | "error"; message: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const check = window.setInterval(() => {
      if (Date.now() - lastActivity > OWNER_SESSION_TIMEOUT_MS) {
        ...
        setError(lang === "ne" ? "????????? ???? ????? ???? ????? ???? ?????? ???? ???? ??????????" : "Owner session was locked for safety. Please log in again.");
      }
    }, 30000);
  ...
  const requestLoginOtp = async () => {
    if (!login.identifier.trim() || !login.password) {
      setError(lang === "ne" ? "????? ??????? ? ??????? ???????????" : "Enter username and password first.");
      return;
    }
    setError("");
    try {
      ...
      if (!response.ok) {
        throw new Error(
          (result as any)?.error ||
          (result as any)?.message ||
          (lang === "ne" ? "???? ??? ????? ??????" : "Failed to send login code."),
        );
      }
  ...
  const requestPasswordReset = async () => {
    if (!forgotForm.identifier.trim()) {
      setError(lang === "ne" ? "????? ?????? ?????? ??????? ???????????" : "Enter an allowed owner username first.");
      return;
    }
    setResetBusy(true);
    setError("");
  ...
  const resetPassword = async (event: React.FormEvent) => {
    ...
    if (!forgotForm.identifier.trim() || !forgotForm.otp.trim() || !forgotForm.newPassword) {
      setError(lang === "ne" ? "???????, ????? ??? ? ???? ??????? ????????" : "Identifier, reset code, and new password are required.");
      return;
    }
    if (forgotForm.newPassword !== forgotForm.confirmPassword) {
      setError(lang === "ne" ? "???? ??????? ? ?????? ??????? ??????" : "New password and confirm password do not match.");
      return;
    }
    setResetBusy(true);
    setError("");
    try {
      ...
      setError(lang === "ne" ? "??????? ????? ???? ?? ???? ????????? ???? ??????????" : "Password reset completed. Log in with the new password now.");
    } catch (err) {
  ...
  const changePassword = async (event: React.FormEvent) => {
    ...
    if (!passwordForm.currentPassword || !passwordForm.newPassword) {
      showOwnerFeedback("error", lang === "ne" ? "????? ? ???? ??????? ???? ????????" : "Both current and new passwords are required.");
      return;
    }
    if (passwordForm.newPassword.length < 6) {
      showOwnerFeedback("error", lang === "ne" ? "???? ??????? ??????? ? ??????? ?????????" : "New password must be at least 6 characters.");
      return;
    }
    if (passwordForm.newPassword !== passwordForm.confirmPassword) {
      showOwnerFeedback("error", lang === "ne" ? "???? ??????? ? ?????? ??????? ??????" : "New password and confirm password do not match.");
      return;
    }
