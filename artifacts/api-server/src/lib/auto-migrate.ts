...
async function copyTable(source: Client, target: Client, table: TableName) {
  logger.info({ table }, "Auto-migrate copying table");
  ...
  if (!sourceRows.rows.length) {
    logger.info({ table, rows: 0 }, "Auto-migrate copied table");
    return;
  }
  ...
  logger.info({ table, rows: sourceRows.rows.length }, "Auto-migrate copied table");
}

let autoMigratePromise: Promise<void> | null = null;

export async function runAutoMigrationIfConfigured(): Promise<void> {
  if (process.env.AUTO_MIGRATE_ON_BOOT !== "1") {
    return;
  }
  ...
  if (!autoMigratePromise) {
    autoMigratePromise = (async () => {
      ...
      if (!sourceUrl) {
        throw new Error("AUTO_MIGRATE_ON_BOOT is enabled but SOURCE_DATABASE_URL is missing");
      }

      if (!targetUrl) {
        throw new Error("AUTO_MIGRATE_ON_BOOT is enabled but DATABASE_URL is missing");
      }
      ...
      try {
        logger.warn("AUTO_MIGRATE_ON_BOOT is enabled. Starting one-time database copy.");
        ...
        logger.warn("Automatic database migration completed successfully.");
      } catch (error) {
